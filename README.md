...
6. **publish-npm** / **publish-gh-packages** — two independent jobs publish the web package (`@worldcoin/nucleus`): the primary to the **public npm registry** (`--access public`; requires an `NPM_TOKEN` repo secret with publish rights to the `@worldcoin` npm org, same as minikit-js / idkit-js) and a backup mirror to **GitHub Packages** (`GITHUB_TOKEN`). They run independently so one failing doesn't block the other; both use `--ignore-scripts`. Android stays on GitHub Packages (Maven).
